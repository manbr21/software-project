...
class ExampleAgent(BaseAgent):
    ...
    def put_obstacles(self): #opponents = obstacles
        obstacles = []
    ...
    def calculate_path(self, obstacles):
        if self.current_index == -1 or self.collide == 1: #if we are looking for a new path
            print("recalculating path" if self.collide == 1 else "ralculating path")
            ...
            if(self.rrt.path == -1): #try again
                ...
                if self.collide == 1:
                    self.collide = 2 #path found, not looking for collision
                    ...
                    if i != 0:
                        ...
                        if Point(future_x, future_y).dist_to(where_to_go) < OBSTACLES_SIZE and will_collide == False: #potential collision and isnt the last node
                            ...
                            if not self.is_last_node():
                                print("possibly a collision")
                                will_collide = True
                                self.collide = 1   
    
    def go_to_node(self, where_to_go):
    ...
    def decision(self):
        ...
        where_to_go = Point(0,0) #initializing
